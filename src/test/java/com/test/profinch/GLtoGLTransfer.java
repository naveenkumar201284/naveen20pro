...
public class GLtoGLTransfer extends Utility{
	
public static final Logger log = Logger.getLogger(GLtoGLTransfer.class.getName());
...
/*			String un = xldata.getCellData("FinFlowLogin", "Username", 2);
	...
	@Test
	public void GltoGlTransferData() throws Exception {
		log.info("This is a test. Don't worry.");
		String TestCaseID = null;

		ss = new ScreenShot();
		pram=new FilloReader().getTestDataInListMap(new File("C:\\Users\\naveen.kumar\\eclipse-workspace\\naveen123\\proFInflow\\src\\main\\java\\datamanager\\loginData.xlsx"),"GLtoGLTransfer", "select * from %s where Runmode='Yes'");
		for (Map<String, String> data : pram) {

			try {

			test=extent.createTest("Testing GL to GL Transfer Screen");
		...
		/*	By by = By.xpath("//mat-card[@class='mat-card']//div//input");
			...
			wait.until(ExpectedConditions.elementToBeClickable(driver.findElement(By.xpath("//div[contains(text(),'GL to GL transfer')]")))).click();
			log.info("Clicked on Cash withdrawal");
				
				Thread.sleep(5000);
				String fromGl=data.get("FromGLNumber");
				if(fromGl != null && !fromGl.isEmpty()) {
				driver.findElement(By.name("glNumber")).sendKeys(fromGl);
				passFailScreenshot("FinFlowz");
				test.pass("Enter the From GL Number");
				log.info(fromGl);
				}
				Thread.sleep(3000);

				
				String fromcurrency=data.get("FromCurrency");
				if(fromcurrency != null && !fromcurrency.isEmpty()) {
				driver.findElement(By.name("glCurrencyCode")).sendKeys(fromcurrency);
				passFailScreenshot("FinFlowz");
				test.pass("Select the From Currency");
				log.info(fromcurrency);
				}
				
				Thread.sleep(3000);

				String glamount=data.get("GLAmount");
				if(glamount != null && !glamount.isEmpty()) {
				driver.findElement(By.xpath("//strong[contains(text(),'From GL :')]/following::input[5]")).sendKeys(glamount);
				passFailScreenshot("FinFlowz");
				test.pass("Enter the GL Amount");
				log.info(glamount);
				}
				Thread.sleep(3000);

				wait.until(ExpectedConditions.elementToBeClickable(driver.findElement(By.xpath("//mat-icon[@class='mat-icon notranslate material-icons mat-icon-no-color'][contains(text(),'navigate_next')]")))).click();
				
				Thread.sleep(3000);
				String toglnumber=data.get("ToGLNumber");
				if(toglnumber != null && !toglnumber.isEmpty()) {
				driver.findElement(By.name("glNumber")).sendKeys(toglnumber);
				passFailScreenshot("FinFlowz");
				test.pass("Enter To GL Number");
				log.info(toglnumber);
				}
				...
				String tocurrency=data.get("ToCurrency");
				if(tocurrency != null && !tocurrency.isEmpty()) {
				driver.findElement(By.name("glCurrencyCode")).sendKeys(tocurrency);
				driver.findElement(By.xpath("//div[@class='mat-body-strong']")).click();
				passFailScreenshot("FinFlowz");
				test.pass("Select To Currency");
				log.info(tocurrency);
				}
				Thread.sleep(2000);	
				...
				String strQuery="Update GLtoGLTransfer Set TranactionID="+"'"+account+"'"+" where TestCaseID="+"'"+TestCaseID+"'";
